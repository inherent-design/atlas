...
class DocumentProcessor:
    ...
    def process_markdown_file(self, file_path: str) -> List[Dict[str, Any]]:
        ...
        if file_size_mb > 10:
            print(f"Warning: Processing large file ({file_size_mb:.1f} MB): {file_path}")
    ...
    def _split_by_headings(self, content: str, file_path: str = "") -> List[Dict[str, str]]:
        ...
        if not headings:
            ...
            if content_size > max_chunk_size * 5:
                print(f"Warning: Large document without headings ({content_size} chars): {file_path}")
    ...
    def process_directory(self, directory: str, recursive: bool = True) -> None:
        ...
        try:
            ...
            print(f"Successfully processed {len(files)} files into {len(all_chunks)} chunks")
            print(f"Added {new_docs} new documents to collection (now contains {final_doc_count} total)")
        except Exception as e:
