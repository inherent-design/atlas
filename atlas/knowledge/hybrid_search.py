...
from atlas.knowledge.retrieval import RetrievalResult, RetrievalFilter, KnowledgeBase
...
class BM25SearchEngine:
    ...
    def search(
        ...
        for term in query_terms:
            ...
            idf = math.log(1 + (self.doc_count - self.doc_freq[term] + self.epsilon) / 
...
class HybridSearchMerger:
    ...
    def _merge_weighted_score(
        ...
        for result in semantic_results:
            ...
            result_id = result.metadata.get("id", result.metadata.get("simple_id", hash(result.content)))
    ...
    def _merge_rank_fusion(
        ...
        for doc_id in all_docs:
            semantic_score = semantic_weight / (k + semantic_ranks.get(doc_id, 1000)) if doc_id in semantic_ranks else 0
            keyword_score = keyword_weight / (k + keyword_ranks.get(doc_id, 1000)) if doc_id in keyword_ranks else 0
...
class HybridSearchEngine:
    ...
    def search(
        ...
        if settings:
            ...
            semantic_weight_val = semantic_weight if semantic_weight is not None else self.semantic_weight
            keyword_weight_val = keyword_weight if keyword_weight is not None else self.keyword_weight
