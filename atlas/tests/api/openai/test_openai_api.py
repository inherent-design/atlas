"""
API tests for OpenAI provider.

This module contains tests that make real API calls to OpenAI.
...
from atlas.tests.helpers.decorators import api_test, openai_test
from atlas.tests.helpers.base_classes import (
    OpenAIProviderTestBase,
...
class TestOpenAIRealAPI(OpenAIProviderTestBase):
    """Test the OpenAI provider with real API calls."""

    provider_class = OpenAIProvider
    provider_name = "openai"
    default_model = "gpt-3.5-turbo"  # Use cheaper model for API tests than the provider default
    ...
    def test_generate_api(self):
        ...
        request = self._create_request(
            "Write a single word response: Hello", max_tokens=5
        )
    ...
    def test_model_selection(self):
        ...
        # Only test with a cheaper model
        model_name = "gpt-3.5-turbo"

        # Create provider with specific model
        provider = self.provider_class(
            model_name=model_name, api_key=os.environ.get("OPENAI_API_KEY")
        )
