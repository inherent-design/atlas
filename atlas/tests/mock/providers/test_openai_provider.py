...
from atlas.tests.helpers import (
    ...
    OpenAIProviderTestBase, TestWithTokenTracking,
...
class TestOpenAIMockProvider(OpenAIProviderTestBase):
    """Test the OpenAIProvider class with mocked responses."""
    ...
    def setUp(self):
        ...
        self.provider = self.provider_class(api_key="test-key", model_name=self.default_model)

    @mock_test
    def test_provider_creation(self):
        """Test creating an OpenAI provider."""
        # Create a provider with explicit model
        provider = self.provider_class(api_key="test-key", model_name=self.default_model)
    ...
    def test_generate_mocked(self):
        ...
        with mock.patch("atlas.models.openai.OpenAI") as mock_openai_class, \
            ...
            mock_completion.model = self.default_model
            ...
            self.assertEqual(kwargs["model"], self.default_model)
            self.assertEqual(kwargs["max_tokens"], 10)  # From _create_request default
    ...
    def test_streaming_mocked(self):
        ...
        stream_handler = StreamHandler(iter(chunks), self.provider, self.default_model, initial_response)
    ...
    def test_stream_with_callback_mocked(self):
        """Test streaming with a callback function using mocked streams."""
        # Create a simple request
    ...
    def test_error_handling(self):
        ...
        provider_with_auth_error = self.provider_class(
            api_key="test-key", 
        ...
        def raise_auth_error(*args, **kwargs):
            # Raise our own error type directly
        ...
        self.assertIn(self.provider_name, str(context.exception).lower())
        
        # Test rate limit error with a different provider
        provider_with_rate_error = self.provider_class(
            api_key="test-key", 
            model_name=self.default_model
        )
        ...
        def raise_rate_error(*args, **kwargs):
            # Create a details dictionary with provider info
