"""
Test the ToolCapableAgent class with mocked provider responses.

This module tests the functionality of the ToolCapableAgent class,
including tool registration, execution, and messaging, using mocked
providers to avoid making real API calls.
"""
...
from atlas.tests.helpers import mock_test, AgentTestBase
...
class TestToolCapableAgent(AgentTestBase):
    """Test cases for the ToolCapableAgent class."""
    
    agent_class = ToolCapableAgent
    ...
    def test_system_prompt_format(self):
        """Test system prompt format with tools."""
        # First register a tool to guarantee it's in the system prompt
        def test_func(x: int, y: str) -> str:
            """Test function with different argument types."""
            return f"{y} repeated {x} times: " + y * x
        
        self.agent.register_tool(test_func)
        
        # Check system prompt format
        system_prompt = self.agent.system_prompt
        
        # Should contain tool section heading (exact section heading from tool_agent.py)
        self.assertIn("## Available Tools", system_prompt)
        
        # Should contain the new tool
        self.assertIn("test_func", system_prompt)
        
        # Should include argument properties in JSON format
        self.assertIn("\"x\": {", system_prompt)
        self.assertIn("\"type\": \"integer\"", system_prompt)
        self.assertIn("\"y\": {", system_prompt)
        self.assertIn("\"type\": \"string\"", system_prompt)
        
        # Should have tool response format instructions
