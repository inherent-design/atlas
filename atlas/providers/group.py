...
from typing import List, Dict, Any, Callable, Optional, Union, Tuple, Set, Type
...
class TaskAwareSelectionStrategy:
    ...
    def select(
        ...
        provider_scores_str = [f"{name}:{score}" for (_, score), name in zip(provider_scores, provider_names)]
        logger.debug(f"Task-aware provider selection for {task_type}: {', '.join(provider_scores_str)}")
...
class ProviderGroup(ModelProvider):
    ...
    def __init__(
        ...
        self._error_counts = {provider: 0 for provider in providers}
        ...
        with self._lock:
            if success:
                ...
                if self._error_counts[provider] >= 3:
                    if self._health_status.get(provider, True):
                        logger.warning(f"Provider {provider.name} marked unhealthy after {self._error_counts[provider]} consecutive errors")
    ...
    def generate(self, request: ModelRequest) -> ModelResponse:
        ...
        if all_errors:
            error_details = "; ".join([f"{getattr(p, 'name', str(p))}: {str(e)}" for p, e in all_errors])
    ...
    def generate_stream(self, request: ModelRequest) -> Tuple[ModelResponse, StreamHandler]:
        ...
        for i, provider in enumerate(providers_to_try):
            try:
                logger.debug(f"Trying provider {provider.name} for streaming ({i+1}/{len(providers_to_try)})")
                ...
                # Record error
                error_msg = f"Provider {provider.name} failed streaming: {str(e)}"
                logger.warning(error_msg)
                all_errors.append((provider, e))
                ...
                if self.retry_policy.enabled and i < len(providers_to_try) - 1:
                    ...
                    if delay > 0:
                        logger.debug(f"Waiting {delay:.2f}s before trying next provider for streaming")
                        time.sleep(delay)
    ...
    def get_available_models(self) -> List[str]:
        ...
        for provider in self.providers:
            try:
                ...
                logger.warning(f"Failed to get models from provider {provider.name}: {str(e)}")
    ...
    def validate_api_key(self) -> bool:
        ...
        for provider in self.providers:
            try:
                ...
                logger.warning(f"Error validating API key for {provider.name}: {str(e)}")
        ...
        for provider in healthy_providers:
            ...
            if hasattr(provider, "get_capability_strength"):
                try:
                    ...
                    logger.warning(f"Error getting capability {capability} from {provider.name}: {str(e)}")
