...
class ProviderRetryConfig:
    ...
    def __init__(
        ...
        retry_error_severities: Optional[List[ErrorSeverity]] = None,
    ):
        """
...
class ProviderCircuitBreaker:
    ...
    def allow_request(self) -> bool:
        ...
        with self._lock:
            ...
            if self._state != self.STATE_CLOSED and (now - self._last_activity_time) > self.reset_timeout:
            ...
            if self._state == self.STATE_OPEN:
                ...
                if now - self._last_failure_time >= self.recovery_timeout:
                    ...
                    logger.debug(f"Circuit breaker blocking request in open state (recovery in {self.recovery_timeout - (now - self._last_failure_time):.1f}s)")
                    return False
            ...
            if self._state == self.STATE_HALF_OPEN:
                ...
                if self._test_requests_remaining > 0:
                    ...
                    logger.debug(f"Circuit breaker allowing test request ({self._test_requests_remaining} remaining)")
                    return True
                logger.debug("Circuit breaker blocking request in half-open state (no test requests remaining)")
                return False
...
class ProviderReliabilityManager:
    ...
    def execute_with_reliability(
        ...
        **kwargs: Any
        ...
        if not self.circuit_breaker.allow_request():
            logger.warning(
                f"{provider_name} provider circuit breaker is open, request blocked"
            )
    ...
    def _should_retry(self, error: Exception) -> bool:
        ...
        if self.retry_config.retry_on_timeout and isinstance(error, (ProviderTimeoutError, TimeoutError)):
            return True
