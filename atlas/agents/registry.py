...
def register_agent_constructor(agent_name: str, constructor: Callable[..., AtlasAgent]) -> None:
    ...
    **kwargs: Any,
    ...
    if agent_name in _AGENT_CONSTRUCTORS:
        ...
        if agent_type and not isinstance(agent, agent_type):
            raise ValueError(f"Custom constructor for '{agent_name}' returned {type(agent)}, expected {agent_type}")
    ...
    if agent_type and not issubclass(agent_class, agent_type):
        raise ValueError(f"Agent class for '{agent_name}' is {agent_class}, not a subclass of {agent_type}")
...
class AgentRegistry(TracedClass):
    ...
    def register(self, agent_name: str, agent_class: Optional[Type[AtlasAgent]] = None, 
                 class_path: Optional[str] = None, 
                 constructor: Optional[Callable[..., AtlasAgent]] = None) -> None:
        ...
        if agent_class:
            ...
            raise ValueError("Must provide one of agent_class, class_path, or constructor")
