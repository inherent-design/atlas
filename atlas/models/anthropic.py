...
class AnthropicProvider(ModelProvider):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if env.get_bool("SKIP_API_KEY_CHECK", False):
            logger.info("Skipping Anthropic API key check due to SKIP_API_KEY_CHECK=true")
        else:
    ...
    def validate_api_key(self) -> bool:
        ...
        if env.get_bool("SKIP_API_KEY_CHECK", False):
            logger.info("Skipping Anthropic API key validation due to SKIP_API_KEY_CHECK=true")
            return True
    ...
    def stream(self, request: ModelRequest) -> Tuple[ModelResponse, Any]:
        ...
        def create_stream():
            ...
            if env.get_bool("SKIP_API_KEY_CHECK", False):
                ...
                def mock_stream_generator():
                    ...
                    class MockResponse:
                        def __init__(self):
                            self.delta = type('obj', (object,), {'text': 'This is a mock response from the Atlas system since SKIP_API_KEY_CHECK is true.'})
                            self.type = 'content'
                ...
                return initial_response, StreamHandler(mock_stream, self, model, initial_response)
            ...
            return initial_response, StreamHandler(stream, self, model, initial_response)
        ...
        try:
            ...
            logger.error(f"Error calling Anthropic API for streaming: {e}", exc_info=True)
            raise  # Re-raise the error after logging it
...
class StreamHandler:
    ...
    def __next__(self):
        ...
        try:
            ...
            logger.error(f"Error getting next chunk from stream: {stream_error}", exc_info=True)
            self.finished = True
