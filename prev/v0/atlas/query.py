...
class AtlasQuery:
    ...
    def query_with_context(
        ...
        documents = self.knowledge_base.retrieve(
            query_text, filter=retrieval_filter, settings=settings
        )
        ...
        try:
            ...
            if filter is not None:
                if isinstance(filter, dict):
                    ...
                    logger.warning(
                        f"Unsupported filter type: {type(filter)}. Converting to string representation."
                    )
                    try:
            ...
            logger.debug(
                f"Streaming query with filter: {retrieval_filter.where if retrieval_filter else None}"
            )
            ...
            if hasattr(self.agent, "process_message_streaming"):
                ...
                return self.agent.process_message_streaming(
                    query_text, callback, filter=retrieval_filter, settings=settings
                )
            else:
    ...
    def retrieve_only(
        ...
        if filter is not None:
            if isinstance(filter, dict):
                ...
                logger.warning(
                    f"Unsupported filter type: {type(filter)}. Using string representation."
                )
                try:
                    # Try to convert to a dictionary if possible
                    filter_dict = {"custom_filter": str(filter)}
        ...
        # Log the filter and settings for debugging
        logger.debug(
            f"Retrieving documents with filter: {retrieval_filter.where if retrieval_filter else None}"
        )
