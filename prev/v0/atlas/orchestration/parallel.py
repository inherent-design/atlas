...
async def run_parallel_tasks(
    workers: dict[str, WorkerAgent], tasks: list[dict[str, Any]]
) -> dict[str, Any]:
    ...
    async def process_worker_tasks(
        worker_id: str, tasks: list[dict[str, Any]]
    ) -> list[dict[str, Any]]:
        worker = None
        for w in workers.values():
            if w.worker_id == worker_id:
        ...
        for task in tasks:
            ...
            with concurrent.futures.ThreadPoolExecutor() as pool:
                result = await loop.run_in_executor(pool, worker.process_task, task)
    ...
    coroutines = [
        process_worker_tasks(worker_id, tasks) for worker_id, tasks in task_assignments.items()
    ]
