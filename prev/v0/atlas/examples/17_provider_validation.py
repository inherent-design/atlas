...
try:
    ...
    from atlas.providers.validation import validate_provider_config
...
def validate_provider_examples():
    ...
    for model_name, provider_type in models_to_validate:
        ...
        print(
            f"Model '{model_name}' for {provider_type}: {highlight('✅ Valid', 'green') if is_valid else highlight('❌ Invalid', 'red')}"
        )
        if error:
...
def main():
    ...
    args = setup_example("Atlas Provider Validation", setup_func=add_cli_arguments)
