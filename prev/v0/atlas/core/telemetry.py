...
def initialize_telemetry(
    ...
    try:
        ...
        logger.error(f"Failed to initialize telemetry: {e!s}")
        _tracer_provider = None
        _meter_provider = None
        _atlas_tracer = None
        _atlas_meter = None
...
def create_counter(name: str, description: str, unit: str = "1") -> Counter | None:
...
def create_histogram(name: str, description: str, unit: str = "ms") -> Histogram | None:
        ...
        def wrapper(*args: Any, **kwargs: Any) -> T:
            ...
            with tracer.start_as_current_span(span_name, attributes=span_attributes) as span:
                try:
                    ...
                    logger.error(f"Exception in {qualname}: {ex!s}", exc_info=True)
                    raise
                finally:
                    if logger.isEnabledFor(logging.DEBUG):
