...
class WorkerAgent(AtlasAgent):
    ...
    def __init__(
        ...
        task_type_mapping: dict[str, list[str]] | None = None,
    ):
    ...
    def process_task(self, task: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            if not task_type and self.task_aware:
                ...
                if detected_task and self.task_type_mapping:
                    ...
                    if self.specialization == "Information Retrieval and Document Summarization":
                        ...
                        self.specialization == "Query Analysis and Information Needs Identification"
                    ):
            ...
            result = self.process_message(
                message=query, task_type=task_type, capabilities=capabilities
            )
            ...
            logging.error(f"Error in worker processing: {e!s}", exc_info=True)
            return {
                "worker_id": self.worker_id,
                "task_id": task.get("task_id", "unknown"),
                "status": "error",
