...
class AtlasAgent(TracedClass):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        try:
            ...
            logger.info(f"Using {provider_display_name} provider with model: {model_display_name}")

            # Set up agent metadata
            self.agent_id = f"atlas-{provider_display_name}-{model_display_name}"

        except Exception as e:
            error_msg = f"Failed to initialize model provider: {e!s}"
    ...
    def process_message(
        ...
        try:
            ...
            documents = self.query_knowledge_base(
                message, filter=retrieval_filter, settings=retrieval_settings
            )
            ...
            if response.usage:
                ...
                try:
                    ...
                    logger.debug(f"Cost formatting error: {cost_err!s}")
    ...
    def process_message_streaming(
        ...
        try:
            ...
            logger.error(f"Error processing streaming message: {e!s}", exc_info=True)
            error_message = (
