...
from atlas.core.errors import APIError, AuthenticationError, ErrorSeverity, ValidationError
...
class ProviderAuthenticationError(AuthenticationError, ProviderError):
    ...
    def __init__(
        ...
        super().__init__(
            message=message, cause=cause, severity=severity, provider=provider, details=details
        )
...
def convert_provider_error(
    ...
    return ProviderError(
        message=f"Error in provider {provider_name}: {error!s}",
        cause=error,
