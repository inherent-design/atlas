...
class OllamaProvider(ModelProvider):
    ...
    # Default timeout for large models (models containing specific identifiers)
    DEFAULT_LARGE_MODEL_TIMEOUT = 300  # 5 minutes

    # Model name patterns indicating large models that need longer timeouts
    LARGE_MODEL_PATTERNS = [":32b", ":34b", ":70b", "llama3:70b", "mixtral", "qwen"]
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        # Determine if this is a large model that needs a longer timeout
        is_large_model = False
        if model_name:
            is_large_model = any(
                pattern in model_name.lower() for pattern in self.LARGE_MODEL_PATTERNS
            )

        # Check if request_timeout is set in options
        options_timeout = None
        if options and isinstance(options, dict) and "request_timeout" in options:
            options_timeout = options.get("request_timeout")
            if options_timeout and isinstance(options_timeout, (int, float)):
                logger.info(f"Using request timeout from options: {options_timeout}s")

        # Get request timeout with priority: explicit param > options > env var > default (with large model check)
        default_timeout = (
            self.DEFAULT_LARGE_MODEL_TIMEOUT if is_large_model else self.DEFAULT_REQUEST_TIMEOUT
        )
        self.request_timeout = (
            request_timeout
            or options_timeout
            or env.get_float("OLLAMA_REQUEST_TIMEOUT", default_timeout)
        )

        # Log timeout settings for large models
        if is_large_model:
            logger.info(
                f"Using extended timeout of {self.request_timeout}s for large model: {model_name}"
            )
    ...
    def _fetch_models(self) -> list[str]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if models:
                    logger.info(
                        f"Discovered {len(models)} models from Ollama server at {self.api_endpoint}"
                    )
                    return models
                else:
    ...
    def validate_api_key(self) -> bool:
        ...
        try:
            ...
            response = requests.get(f"{self.api_endpoint}/version", timeout=self.connect_timeout)
            if response.status_code == 200:
                return True
            else:
            ...
            logger.warning(f"Error connecting to Ollama server at {self.api_endpoint}: {e}")
            return False
    ...
    def validate_api_key_detailed(self) -> dict[str, Any]:
        ...
        return {
            ...
            "provider": self.name,
            "error": (
                None if is_valid else f"Ollama server at {self.api_endpoint} is not accessible"
            ),
        }

    def _convert_messages(self, messages: list[ModelMessage]) -> tuple[str, str | None]:
    ...
    def _should_retry_request(self, request: ModelRequest) -> bool:
        ...
        if retry_state.get("count", 0) >= retry_config.max_retries:
            logger.debug(
                f"Maximum retries ({retry_config.max_retries}) reached for request {request_id}"
            )
            return False
        ...
        try:
            ...
            for key, value in self.options.items():
                if (
                    key not in params
                    and key != "options"
                    and key not in ["max_tokens", "temperature", "capabilities"]
                ):
                    params[key] = value
            ...
            logger.debug(
                f"Making Ollama API request to model {model} with timeout {self.request_timeout}s"
            )
            ...
            # Set default values for missing token counts to avoid validation errors
            ...
            if not hasattr(usage, "total_tokens") or usage.total_tokens is None:
            ...
            try:
                ...
                logger.warning(f"Token usage validation failed: {e}, using default values")
                # On validation error, create a default TokenUsage
            ...
            if isinstance(e, requests.ConnectionError):
                error_message = f"Failed to connect to Ollama server at {self.api_endpoint}: {e}"
        ...
        try:
            ...
            for key, value in self.options.items():
                if (
                    key not in params
                    and key != "options"
                    and key not in ["max_tokens", "temperature", "capabilities", "stream"]
                ):
                    params[key] = value
                elif key == "options" and isinstance(value, dict):
                    for option_key, option_value in value.items():
            ...
            # Log the request details for debugging
            logger.debug(
                f"Making Ollama streaming API request to model {model} with timeout {self.request_timeout}s"
            )
    ...
    def calculate_token_usage(self, request: ModelRequest, response: Any) -> TokenUsage:
        ...
        if prompt_tokens == 0 and hasattr(request, "messages") and request.messages:
            # Rough estimate: ~4 characters per token
            prompt_text = ""
            ...
            prompt_tokens = max(1, len(prompt_text) // 4)  # Ensure at least 1 token

        # If eval tokens is 0, estimate based on character count
        if eval_tokens == 0 and "response" in response:
            # Rough estimate: ~4 characters per token
            eval_tokens = max(1, len(response["response"]) // 4)  # Ensure at least 1 token

        # Ensure we have valid values
        prompt_tokens = max(0, prompt_tokens)
        eval_tokens = max(0, eval_tokens)
        total_tokens = prompt_tokens + eval_tokens

        # Log token usage estimates
        logger.debug(
            f"Ollama token usage estimates: input={prompt_tokens}, output={eval_tokens}, total={total_tokens}"
        )
