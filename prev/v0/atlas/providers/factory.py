...
from atlas.providers.events import create_event_enabled_provider
...
_AVAILABLE_PROVIDERS: dict[str, list[str]] | None = None
...
def detect_model_provider(model_name: str) -> str | None:
...
def get_model_by_capability(provider_name: str, capability: str = DEFAULT_CAPABILITY) -> str:
    ...
    if provider_name in PROVIDER_MODELS:
        ...
        logger.debug(
            f"No model found for capability {capability}, using default: {PROVIDER_MODELS[provider_name]['default']}"
        )
        ...
        try:
            ...
            try:
                ...
                if response.status_code == 200:
                        ...
                        if models_response.status_code == 200:
                            ...
                            if ollama_models:
                                ...
                                logger.debug(
                                    ...
                                    + (
                                        f" and {len(ollama_models) - 5} more"
                                ...
                                providers["ollama"] = list(
                                    PROVIDER_MODELS["ollama"]["models"].keys()
                                )
                                logger.warning(
                                    "Ollama server returned no models, using known models as fallback"
                                )
                        else:
                    ...
                    logger.warning(
                        f"Ollama server not responsive: status code {response.status_code}"
                    )
...
def register_provider(provider_name: str, class_path: str) -> None:
    ...
    try:
        ...
        registry.register_provider(name=provider_name, provider_class=provider_class)
...
def create_provider(
    ...
    **kwargs: Any,
    ...
    Args:
        ...
        event_system: Optional event system instance to use for event tracking.
        enable_events: Whether to enable event tracking for this provider.
    ...
    try:
        ...
        if registry:
            ...
            if model_name:
                ...
                return registry.create_provider(
                    provider_name=provider_name, max_tokens=max_tokens, **kwargs
                )
    except Exception as e:
        logger.warning(
            f"Registry-based provider creation failed, falling back to direct creation: {e}"
        )
    ...
    if model_name is not None and provider_name is None:
        ...
        if detected_provider:
            ...
            logger.debug(
                f"Could not detect provider from model '{model_name}', using default: {provider_name}"
            )
    ...
    if model_name is None:
        ...
        if capability is not None:
            ...
            if env_model:
                ...
                logger.debug(
                    f"Using model '{model_name}' from environment variable {env_model_var}"
                )
            else:
                ...
                if general_default and is_model_compatible_with_provider(
                    general_default, provider_name
                ):
    ...
    if "mock" in model_name.lower() and provider_name != "mock":
        logger.warning(
            f"Model '{model_name}' appears to be a mock model but provider is '{provider_name}'."
        )
        logger.warning("Switching provider to 'mock' to match model")
        provider_name = "mock"
    ...
    try:
        ...
        try:
            ...
            options_schema = PROVIDER_OPTIONS_SCHEMAS.get(
                provider_name, PROVIDER_OPTIONS_SCHEMAS["default"]
            )
        ...
        # Add event system if requested
        if enable_events:
            # Use provided event system or create a new one
            event_sys = event_system or create_event_system()

            # Create event-enabled provider
            provider = create_event_enabled_provider(
                provider_class=provider.__class__, event_system=event_sys, **provider_kwargs
            )
            logger.debug(f"Enhanced provider '{provider_name}' with event system")
...
def create_provider_group(
    providers: list[str | ModelProvider],
    ...
    **kwargs: Any,
    ...
    """
    from atlas.providers.group import (
        ProviderGroup,
        ProviderSelectionStrategy,
        TaskAwareSelectionStrategy,
    )
    ...
    return ProviderGroup(
        providers=provider_instances, selection_strategy=strategy_function, name=name
    )
        ...
        try:
            if skip_validation:
                ...
                results[provider_name] = {
                    ...
                    "error": (None if api_key else f"No API key found for {provider_name}"),
                }
            else:
...
class ProviderFactory(TracedClass):
    ...
    def __init__(self, event_system: EventSystem | None = None):
        """Initialize the provider factory.

        Args:
            event_system: Optional event system to use for all providers.
        """
        self._providers: dict[str, ModelProvider] = {}
        self._default_provider: str | None = None
        self._event_system = event_system or create_event_system()
        super().__init__()
    ...
    def create_group(
        ...
        **kwargs: Any,
        ...
        group = create_provider_group(
            providers=provider_instances, selection_strategy=selection_strategy, name=name
        )
    ...
    def get(self, provider_name: str | None = None) -> ModelProvider:
