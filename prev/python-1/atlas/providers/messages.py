...
class MessageContent:
    ...
    def __init__(self, type: str, text: str | None = None, image_url: dict[str, Any] | None = None):
    ...
    @classmethod
    def create_direct(
        cls, type: str, text: str | None = None, image_url: dict[str, Any] | None = None
    ) -> "MessageContent":
    ...
    def create_text(cls, text: str) -> "MessageContent":
        ...
        return cls.create_direct(type="text", text=text)
    ...
    def create_image_url(cls, url: str, detail: str = "auto") -> "MessageContent":
        ...
        """
        # Use create_direct to avoid schema validation
        return cls.create_direct(type="image_url", image_url={"url": url, "detail": detail})
    ...
    def from_dict(cls, data: dict[str, Any]) -> "MessageContent":
        ...
        if content_type == "text":
            return cls.create_direct(type="text", text=data.get("text", ""))
        elif content_type == "image_url":
            return cls.create_direct(type="image_url", image_url=data.get("image_url", {}))
        else:
            # Generic handler for unknown types
            return cls.create_direct(
                type=content_type, text=data.get("text"), image_url=data.get("image_url")
            )
...
class ModelMessage:
    ...
    def system(cls, content: str) -> "ModelMessage":
        ...
        """
        # Use create_direct to avoid schema validation
        return cls.create_direct(role=MessageRole.SYSTEM, content=str(content), name=None)
    ...
    def user(
        ...
        return cls.create_direct(role=MessageRole.USER, content=processed_content, name=name)
    ...
    def assistant(
        ...
        # Use create_direct method with processed content
        return cls.create_direct(role=MessageRole.ASSISTANT, content=processed_content, name=name)
    ...
    def function(cls, content: str, name: str) -> "ModelMessage":
        ...
        return cls.create_direct(role=MessageRole.FUNCTION, content=str(content), name=name)
    ...
    def tool(cls, content: str, name: str) -> "ModelMessage":
        ...
        """
        # Use create_direct method
        return cls.create_direct(role=MessageRole.TOOL, content=str(content), name=name)
    ...
    def from_dict(cls, data: dict[str, Any]) -> "ModelMessage":
        ...
        return cls.create_direct(role=role, content=content, name=name)

    @classmethod
    def _process_content(
        cls, content: str | MessageContent | list[Any]
...
class ModelRequest:
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        if hasattr(self, "messages") and self.messages:
            result["messages"] = [
                msg.to_dict() if hasattr(msg, "to_dict") else msg for msg in self.messages
            ]
    ...
    def to_provider_request(self, provider_name: str) -> dict[str, Any]:
        ...
        if provider_name.lower() == "anthropic":
            ...
            if system_messages:
                ...
                request["messages"] = [
                    msg.to_dict() for msg in self.messages if msg.role != MessageRole.SYSTEM
                ]
...
class TokenUsage:
    ...
    def __init__(
        self, input_tokens: int = 0, output_tokens: int = 0, total_tokens: int | None = None
    ):
        ...
        self.input_tokens = int(input_tokens) if input_tokens is not None else 0
        self.output_tokens = int(output_tokens) if output_tokens is not None else 0
    ...
    @classmethod
    def create_direct(
        cls, input_tokens: int = 0, output_tokens: int = 0, total_tokens: int | None = None
    ) -> "TokenUsage":
        ...
        # Ensure integer values
        instance.input_tokens = int(input_tokens) if input_tokens is not None else 0
        instance.output_tokens = int(output_tokens) if output_tokens is not None else 0

        if total_tokens is not None:
            instance.total_tokens = int(total_tokens)
        else:
    ...
    @classmethod
    def from_provider_response(cls, response: Any, provider_name: str) -> "TokenUsage":
        """Create a TokenUsage instance from a provider-specific response object.
        ...
        Returns:
            A TokenUsage instance with extracted token data.
        """
        ...
        if provider_name.lower() == "openai":
            # For OpenAI, check different response formats
            ...
            # For Anthropic, check usage attribute or raw dictionary
        ...
        # For any provider, if we have tokens but no total, calculate it
        if total_tokens == 0 and (input_tokens > 0 or output_tokens > 0):
            total_tokens = input_tokens + output_tokens

        # Always use create_direct to bypass validation
        return cls.create_direct(
...
class CostEstimate:
    ...
    def __init__(
        self, input_cost: float = 0.0, output_cost: float = 0.0, total_cost: float | None = None
    ):
    ...
    @classmethod
    def create_direct(
        cls, input_cost: float = 0.0, output_cost: float = 0.0, total_cost: float | None = None
    ) -> "CostEstimate":
