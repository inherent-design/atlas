...
def validated_by(schema: Schema, field_name: str | None = None) -> Callable[[F], F]:
...
def provider_schema_validated(
    schema_factory: Callable[[], SchemaType],
    ...
    def decorator(cls: type[T]) -> type[T]:
        ...
        def __init__(self: Any, *args: Any, **kwargs: Any) -> None:
            ...
            params = {
                **{
                    name: value
                    for name, value in zip(list(sig.parameters.keys())[1:], args, strict=False)
                },
                **kwargs,
            }
    ...
    def wrapper(self: Any, *args: Any, **kwargs: Any) -> Any:
        ...
        if hasattr(result, "get_iterator") and hasattr(result, "process_stream"):
            ...
            raise ProviderValidationError(
                ...
                provider=provider_name,
                details={
                    "error": "Stream handler must implement get_iterator and process_stream methods"
                },
            )
...
def validate_capabilities(
    required_capabilities: list[str] = None, min_strengths: dict[str, int] = None
) -> Callable[[F], F]:
