...
def retrieve_knowledge(state: AgentState, config: AtlasConfig | None = None) -> AgentState:
    ...
    try:
        ...
        print(f"Error retrieving knowledge: {e!s}")
        state.error = f"Knowledge retrieval error: {e!s}"
...
def generate_response(
    ...
    try:
        ...
        state.error = f"Response generation error: {e!s}"
    ...
    for task in tasks:
        ...
        if worker_id not in state.workers:
            ...
            worker_state = AgentState(
                worker_id=worker_id, messages=[{"role": "user", "content": user_query}]
            )
        ...
        if worker_state:
            ...
            for message in reversed(worker_state.messages):
                if message["role"] == "assistant":
                    combined_results.append({"worker_id": worker_id, "content": message["content"]})
                    break
...
def generate_final_response(
    ...
    try:
        ...
        print(f"Error generating final response: {e!s}")
        state.error = f"Final response generation error: {e!s}"
...
def should_end(state: AgentState | ControllerState) -> bool:
