...
def add_example_arguments(parser):
    ...
    parser.add_argument(
        "--providers", help="Comma-separated list of providers to include (default: all available)"
    )
...
def demonstrate_task_detection() -> None:
    ...
    for expected_task, prompt in example_prompts.items():
        ...
        capability_str = ", ".join(
            [f"{cap}:{strength.name}" for cap, strength in capabilities.items()]
        )
...
def demonstrate_task_aware_selection(provider_group: ModelProvider) -> None:
    ...
    tasks = [
        ...
        {"name": "Creative Writing", "prompt": "Write a haiku poem about artificial intelligence."},
        {"name": "Math Problem", "prompt": "If x² + 5x + 6 = 0, what are the values of x?"},
        {"name": "Simple Conversation", "prompt": "What are your capabilities?"},
    ]
    ...
    for i, task in enumerate(tasks, 1):
        ...
        print(
            f"Required capabilities: {', '.join([f'{cap}:{strength.name}' for cap, strength in capabilities.items()])}"
        )
