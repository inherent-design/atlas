...
from atlas.agents.specialized.tool_agent import ToolCapableAgent, create_tool_agent
    ...
    try:
        ...
        try:
            ...
            toolkit.grant_permission(
                agent_id=worker_id, tool_name="calculator", granted_by="admin", scope="execute"
            )
            logger.info(
                f"Granted 'execute' permission for 'calculator' tool to agent '{worker_id}'"
            )

            # 2. Grant weather tool permission with system as granter and execute scope
            toolkit.grant_permission(
                agent_id=worker_id, tool_name="weather", granted_by="system", scope="execute"
            )
            ...
            toolkit.grant_permission(
                agent_id=worker_id, tool_name="calculator", granted_by="system", scope="read"
            )
        ...
        sorted_history = sorted(
            permission_history, key=lambda x: (x.get("tool_name", ""), x.get("scope", ""))
        )
        ...
        print(
            f"  - has execute permission for calculator: {toolkit.has_permission(worker_id, 'calculator', 'execute')}"
        )
        print(
            f"  - has read permission for calculator: {toolkit.has_permission(worker_id, 'calculator', 'read')}"
        )
        print(
            f"  - has manage permission for calculator: {toolkit.has_permission(worker_id, 'calculator', 'manage')}"
        )

        # Check for weather permissions with different scopes
        print(
            f"  - has execute permission for weather: {toolkit.has_permission(worker_id, 'weather', 'execute')}"
        )
        print(
            f"  - has read permission for weather: {toolkit.has_permission(worker_id, 'weather', 'read')}"
        )
        ...
        print(
            f"📖 Tools accessible with 'read' scope: {', '.join(readable_tools.keys()) if readable_tools else 'None'}"
        )
