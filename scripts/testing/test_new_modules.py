...
def test_telemetry():
    ...
    logger.info(f"Telemetry enabled after setting ATLAS_ENABLE_TELEMETRY=0: {is_telemetry_enabled()}")
    ...
    shutdown_telemetry()
    initialize_telemetry(service_name="test_service_env")
    logger.info(f"Telemetry enabled after setting ATLAS_ENABLE_TELEMETRY=1: {is_telemetry_enabled()}")
...
if __name__ == "__main__":
    success = run_tests()
    logger.info(f"All tests completed with overall result: {'PASS' if success else 'FAIL'}")
