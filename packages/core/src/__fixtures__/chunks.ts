...
import { liveFixtures } from './live-data/loader.js'

/**
 * Create a ChunkPayload with sensible defaults and optional overrides.
 *
 * Uses structure from real live data to ensure mocks match production reality.
 *
...
export function createChunkPayload(overrides?: Partial<ChunkPayload>): ChunkPayload {
  // Use structure from first live fixture point as template
  const liveTemplate = liveFixtures.qdrant.points[0]?.payload || {}
  ...
  const defaults: ChunkPayload = {
    ...
    // Include fields from live data structure
    embedding_model: liveTemplate.embedding_model || 'voyage-context-3',
    embedding_strategy: liveTemplate.embedding_strategy || 'contextualized',
    content_type: liveTemplate.content_type || 'text',
    vectors_present: liveTemplate.vectors_present || ['text'],
  }
...
export const CHUNKS = {
  ...
  markdown: createChunkPayload({
    original_text:
      'Memory consolidation occurs during sleep, transforming episodic memories into semantic knowledge.',
  ...
  typescript: createChunkPayload({
    original_text:
      'export function embedText(input: string): Promise<number[]> {\n  return voyage.embed(input)\n}',
  ...
  consolidated: createChunkPayload({
    original_text:
      'Sleep consolidation: Episodic memories are replayed and integrated into semantic networks during sleep.',
  ...
  important: createChunkPayload({
    original_text:
      'CRITICAL: Memory corruption detected in consolidation pipeline. Requires immediate investigation.',
  ...
  topicSummary: createChunkPayload({
    original_text:
      'Memory consolidation research shows that sleep plays a crucial role in transforming short-term episodic memories into long-term semantic knowledge through repeated replay and integration.',
  ...
  withCausalLinks: createChunkPayload({
    original_text:
      'Named vectors implementation allows separate embeddings for text, code, and media content.',
