...
bun atlas-ingest.ts /path/to/file.md
```

### Ingest Directory (Recursive)
```bash
cd ~/.claude/skills/atlas/scripts
bun atlas-ingest.ts /path/to/docs/ --recursive
```

### Ingest Multiple Paths
```bash
cd ~/.claude/skills/atlas/scripts
bun atlas-ingest.ts README.md src/index.ts docs/ --recursive
```
...
```bash
cd ~/.claude/skills/atlas/scripts
bun atlas-search.ts "typescript patterns"
```

### Limited Results
```bash
cd ~/.claude/skills/atlas/scripts
bun atlas-search.ts "memory consolidation" --limit 10
```

### Temporal Filtering (Since Date)
```bash
cd ~/.claude/skills/atlas/scripts
bun atlas-search.ts "sleep patterns" --since "2025-12-25"
```

### Chronological Timeline
```bash
cd ~/.claude/skills/atlas/scripts
bun atlas-search.ts --timeline --since "2025-12-01"
```
...
- [INGEST_DETAILS.md](INGEST_DETAILS.md) - Chunking, embedding, storage pipeline
- [SEARCH_DETAILS.md](SEARCH_DETAILS.md) - Semantic + temporal query patterns
- [ARCHITECTURE.md](ARCHITECTURE.md) - Complete technical architecture

Scripts: `~/.claude/skills/atlas/scripts/`
