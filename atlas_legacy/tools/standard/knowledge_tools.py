...
class MetadataFilterTool(Tool):
    ...
    @traced(name="execute")
    def execute(
        self, metadata_filters: dict[str, Any], query: str | None = None, max_results: int = 10
    ) -> list[dict[str, Any]]:
        ...
        if query:
            ...
            results = self.knowledge_base.retrieve(
                query=query, filter=filter_obj, settings=settings
            )
        else:
            # Get all documents matching filter
            results = self.knowledge_base.get_documents(filter=filter_obj, limit=max_results)
...
def create_knowledge_toolkit(knowledge_base: KnowledgeBase) -> list[Tool]:
    ...
    """
    return [KnowledgeSearchTool(knowledge_base), MetadataFilterTool(knowledge_base)]
