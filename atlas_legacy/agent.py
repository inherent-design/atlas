...
def load_system_prompt(file_path: str | None = None) -> str:
    ...
    if file_path and os.path.exists(file_path):
        try:
            ...
            print(f"Error loading system prompt from {file_path}: {e!s}")
...
class AtlasAgent:
    ...
    def query_knowledge_base(self, query: str) -> list[dict[str, Any]]:
        ...
        documents: list[dict[str, Any]] = updated_state.get("context", {}).get("documents", [])
        return documents
...
def parse_args():
    ...
    parser.add_argument("-s", "--system-prompt", type=str, help="Path to system prompt file")
    parser.add_argument(
        "-c",
        "--collection",
        type=str,
    ...
    parser.add_argument("--db-path", type=str, help="Path to ChromaDB database directory")

    # Ingestion options
    parser.add_argument("-d", "--directory", type=str, help="Directory to ingest documents from")
    parser.add_argument(
    ...
    # Query options
    parser.add_argument("-q", "--query", type=str, help="Single query to process (query mode only)")
...
def run_worker_mode(args):
    ...
    try:
        ...
        worker: AnalysisWorker | DraftWorker | RetrievalWorker
