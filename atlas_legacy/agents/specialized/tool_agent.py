    ...
    def __init__(
        ...
        if provider_options:
            ...
            provider_options.require_capability(
                Capability.TOOL_USE, min_level=CapabilityLevel.STRONG
            )

            # Add JSON output capability for proper tool call parsing
            provider_options.require_capability(
                Capability.JSON_OUTPUT, min_level=CapabilityLevel.MODERATE
            )
        ...
        if provider_options and not self.provider:
            ...
            logger.info(
                f"Resolved provider {self.provider.name} with model {self.provider.model_name}"
            )
...
"""
    ...
    def process_structured_message(self, message: StructuredMessage) -> StructuredMessage:
        ...
        try:
                ...
                for tool_call in tool_calls:
                    try:
                        ...
                        tool_result = self.toolkit.execute_tool(
                            agent_id=self.worker_id, tool_name=tool_name, args=tool_args
                        )

                        # Add the result to the response
                        call_id = tool_call.get("id") or "unknown"
                        response.add_tool_result(
                            name=tool_name, result=tool_result, call_id=call_id
                        )
            ...
            return StructuredMessage.create_error(f"Error processing message: {e!s}", message)
    ...
    def _extract_tool_calls_from_content(self, content: str) -> list[dict[str, Any]]:
        ...
        while True:
            ...
            try:
                ...
                logger.warning(f"Failed to parse JSON block: {e!s}")
...
"""
        ...
        final_response = self.process_message(
            message=f"{query}\n\n{follow_up_message}", capabilities=capabilities
        )
...
def create_tool_agent(
    ...
    if provider_name:
        provider_options = ProviderOptions(
            provider_name=provider_name, model_name=model_name, streaming=streaming
        )

        # Require tool use capability
        provider_options.require_capability(Capability.TOOL_USE, min_level=CapabilityLevel.STRONG)
