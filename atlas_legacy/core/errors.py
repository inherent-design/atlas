...
class BufferError(AtlasError):
    """Error related to buffer operations."""
...
class ClosedBufferError(BufferError):
    """Error indicating an operation on a closed buffer."""

    def __init__(
        self,
        message: str = "Operation attempted on a closed buffer",
        details: dict[str, Any] | None = None,
        cause: Exception | None = None,
        severity: ErrorSeverity = ErrorSeverity.ERROR,
    ):
    ...
    try:
        ...
        if error_handler:
            try:
                ...
                logger.error(f"Error in error handler: {handler_error!s}")
