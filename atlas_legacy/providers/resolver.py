...
def resolve_provider_options(options: ProviderOptions) -> ProviderOptions:
    ...
    if resolved.model_name and not resolved.provider_name:
        ...
        if detected_provider:
            ...
            logger.debug(
                f"Auto-detected provider '{detected_provider}' from model '{resolved.model_name}'"
            )
    ...
    if (
        resolved.model_name
        and "mock" in resolved.model_name.lower()
        and resolved.provider_name != "mock"
    ):
        logger.warning(
            f"Auto-correcting provider from {resolved.provider_name} to 'mock' based on mock model name"
        )
    ...
    if resolved.provider_name and resolved.capability and not resolved.model_name:
        ...
        logger.debug(
            f"Selected model '{resolved.model_name}' for provider '{resolved.provider_name}' based on capability '{resolved.capability}'"
        )
    ...
    if not resolved.model_name:
        ...
        if resolved.provider_name:  # Only try if we have a provider
            ...
            if env_model:
                ...
                logger.debug(
                    f"Using provider-specific default model '{resolved.model_name}' from env"
                )
        ...
        if not resolved.model_name:
            ...
            if env_model and (
                not resolved.provider_name
                or is_model_compatible_with_provider(env_model, resolved.provider_name)
            ):
                resolved.model_name = env_model
        ...
        if not resolved.model_name and resolved.provider_name:
            capability = resolved.capability or env.get_string(
                "ATLAS_DEFAULT_CAPABILITY", "inexpensive"
            )
            model = get_model_by_capability(resolved.provider_name, capability)
            resolved.model_name = model
            logger.debug(
                f"Selected model '{resolved.model_name}' based on capability '{capability}'"
            )
