...
class StreamBuffer:
    ...
    @validate_streaming_config
    def __init__(self, max_buffer_size: int = 1024 * 1024, config: dict[str, Any] | None = None):
        """
        ...
        if config is None:
            config = {"max_buffer_size": max_buffer_size, "paused": False, "closed": False}
...
class RateLimitedBuffer(StreamBuffer):
    ...
    def tokens_per_second(self, value: float | None) -> None:
