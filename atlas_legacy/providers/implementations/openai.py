...
class OpenAIStreamHandler(StreamHandler):
    ...
    def process_stream(self, callback: Callable[[str, Any], None] | None = None) -> ModelResponse:
        ...
        try:
            ...
            if not self.response.usage or (
                self.response.usage.input_tokens == 0 and self.response.usage.output_tokens == 0
            ):
                ...
                output_tokens = (
                    len(self.response.content) // 4 if hasattr(self.response, "content") else 0
                )
            ...
            if self.response.usage and (
                not self.response.cost
                or (self.response.cost.input_cost == 0 and self.response.cost.output_cost == 0)
            ):
                if hasattr(self.provider, "calculate_cost"):
                    try:
                        ...
                        logger.debug(
                            f"Calculated final cost in process_stream: {self.response.cost}"
                        )
    ...
    def __init__(
        ...
        super().__init__(
            content="", provider=provider, model=model, initial_response=initial_response
        )
    ...
    def _stream_content(self) -> None:
        ...
        try:
            # Set the state to active - with proper locking
                ...
                for chunk in self.iterator:
                    ...
                    if hasattr(chunk, "usage") and chunk.usage:
                        ...
                        self.response.usage = TokenUsage.from_provider_response(
                            response=chunk, provider_name="openai"
                        )
    ...
    def get_iterator(self) -> Iterator[str | tuple[str, Any]]:
        ...
        while not self._done.is_set() or self._content_buffer:
            ...
            if chunk:
                ...
                # Check the state of the stream with proper locking
                ...
                # Calculate final cost if this is the last chunk
                if is_final_chunk and hasattr(self.provider, "calculate_cost"):
                    try:
                        self.response.cost = self.provider.calculate_cost(
                            self.response.usage, self.model
                        )
        ...
        **kwargs: Any,
        ...
        if not OPENAI_AVAILABLE:
            raise ProviderValidationError(
                "OpenAI SDK not installed. Install with 'uv add openai'", provider="openai"
            )
        ...
        for message in messages:
            ...
            if isinstance(message.content, str):
                    ...
                    if isinstance(block, dict):
                        if block.get("type") == "text":
                            ...
                            content_blocks.append(
                                {"type": "image_url", "image_url": {"url": block.get("url", "")}}
                            )
                ...
                if content_blocks:
                    ...
                    message_dict = {"role": role, "content": content_blocks}
    ...
    def generate(self, request: ModelRequest) -> ModelResponse:
        ...
        try:
            ...
            if hasattr(e, "status_code"):
                ...
                if status_code == 401:
                    raise ProviderAuthenticationError(
                        error_message, provider="openai", details=details
                    ) from e
            ...
            raise ProviderError(
                error_message, provider="openai", details={"original_error": str(e)}
            ) from e
        except Exception as e:
