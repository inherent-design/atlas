...
def add_cli_arguments(parser):
    ...
    parser.add_argument(
        "--skip-api-call",
        action="store_true",
        help="Skip examples that make API calls"
    )
...
def demo_provider_resolution():
    ...
    for capability in capabilities:
        ...
        resolved = resolve_provider_options(options)
        print(f"Provider '{provider}' with capability '{capability}' -> Model: {resolved.model_name}")
...
def main():
    ...
    args = setup_example(
        "Atlas Provider Selection Example",
