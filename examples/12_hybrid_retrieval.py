...
def format_result(result: RetrievalResult, index: int) -> str:
    ...
    return (
        f"{index+1}. {source} (score: {result.relevance_score:.4f})\n"
        f"   {content_snippet}\n"
    )
...
def print_result_overlap(
    ...
    table = [
        ...
        ["Semantic & Keyword", semantic_keyword_overlap, f"{100 * semantic_keyword_overlap / max(1, total_unique):.1f}%"],
        ["Semantic & Hybrid", semantic_hybrid_overlap, f"{100 * semantic_hybrid_overlap / max(1, total_unique):.1f}%"],
        ["Keyword & Hybrid", keyword_hybrid_overlap, f"{100 * keyword_hybrid_overlap / max(1, total_unique):.1f}%"],
...
def main():
    ...
    try:
        ...
        print(f"Retrieved: {len(semantic_results)} semantic, {len(keyword_results)} keyword, and {len(hybrid_results)} hybrid results")
        ...
        print("---------------------------")
        print("🔹 Semantic Search: Best for conceptual queries where exact keywords might not appear")
        ...
        print()
        print("🔹 Hybrid Search: Best general-purpose approach that combines strengths of both methods")
        print("   Example: Any query where both semantic understanding and keyword matching are valuable")
        print()
