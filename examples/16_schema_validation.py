...
from examples.common import (
    setup_example, print_section, print_example_footer, highlight
)
...
from atlas.schemas.messages import (
    ModelMessageSchema, TextContentSchema,
    text_content_schema, model_message_schema,
    validate_message_param, validate_messages_param, validate_content_param
)
from atlas.schemas.providers import (
    TokenUsageSchema, ModelRequestSchema, ModelResponseSchema,
    token_usage_schema, model_request_schema, model_response_schema
)
from atlas.schemas.options import (
    ProviderOptionsSchema, provider_options_schema
)
...
from atlas.schemas.knowledge import (
    ChunkingStrategyEnum, DocumentChunkSchema, RetrievalSettingsSchema,
    chunking_config_schema, document_chunk_schema, retrieval_settings_schema
)
from atlas.schemas.validation import (
    validate_with_schema, validate_args, validate_class_init, create_schema_validated
)
...
from atlas.providers.validation import (
    validate_request, validate_response, validate_capabilities
)
...
def main() -> None:
    ...
    args = setup_example(
        ...
        setup_func=setup_argument_parser
    )
    ...
    def process_message(message: Any) -> Dict[str, Any]:
        ...
        if hasattr(message.content, "text"):
            content_preview = message.content.text[:30] + "..." if len(message.content.text) > 30 else message.content.text
        else:
    ...
    def process_request(request: Dict[str, Any], log_level: str = "info") -> Dict[str, Any]:
        ...
        try:
            ...
            if "messages" not in request or not isinstance(request["messages"], list) or not request["messages"]:
    ...
    try:
        ...
        image_content_data = {"type": "image_url", "image_url": {"url": "https://example.com/image.jpg"}}
    ...
    class MockProvider:
        ...
        def generate(self, request: Any) -> Dict[str, Any]:
            ...
            model = request.model if hasattr(request, "model") else request.get("model", "default-model")
