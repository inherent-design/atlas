...
def create_providers_for_example() -> List[ModelProvider]:
    ...
    return create_providers_for_examples(
        ensure_mock=True,
        min_providers=2
    )
...
def demonstrate_fallback(
    ...
    if len(providers) > 1:
        ...
        try:
            response = provider_group.generate(request)
            print(highlight(f"Response: {response.content[:100]}...", color="green"))
            print(highlight("Fallback successful! The request was handled by a subsequent provider.", color="green"))
        except Exception as e:
...
def demonstrate_strategy(
    ...
    for i, template in enumerate(request_templates[:3], 1):
        ...
        request = ModelRequest(
            messages=[
                ModelMessage.system("You are a helpful assistant. Keep responses very short and concise."),
        ...
        try:
            ...
            # For task-aware strategy, explain provider selection
                ...
                print(f"The request was detected as a specific task type suitable for the selected provider.")
...
def main():
    ...
    try:
        ...
        if len(providers) < 2:
            ...
            print(highlight("Error: At least 2 providers are required for this example.", color="red"))
            return 1
