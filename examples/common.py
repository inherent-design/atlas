...
from atlas.core import env, logging
...
def get_base_argument_parser(description: str) -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--provider", type=str, help="Model provider to use (e.g., anthropic, openai, ollama, mock)"
    )
    parser.add_argument(
        "--model", type=str, help="Specific model to use (auto-detects provider if not specified)"
    )
    parser.add_argument(
        "--capability",
        type=str,
    ...
    parser.add_argument(
        "--timeout",
        type=int,
        help="Request timeout in seconds for provider API calls (particularly useful for Ollama with large models)",
    )
...
def ensure_example_data(
    args: argparse.Namespace, force_ingestion: bool = False
) -> Tuple[bool, int]:
    ...
    is_using_example_data = (
        not args.use_system_db
        and (args.db_path == get_example_db_path() or args.db_path is None)
        and (args.collection == get_example_collection() or args.collection is None)
    )
    ...
    processor = DocumentProcessor(collection_name=args.collection, db_path=args.db_path)
    ...
    if num_processed > 0:
        ...
        with open(DATA_MARKER_FILE, "w") as f:
            f.write(
                f"Data ingested at {os.path.basename(EXAMPLE_DOCS_PATH)} with {num_processed} files"
            )
...
def create_provider_from_args(args: argparse.Namespace) -> Any:
    ...
    # Create a dictionary to pass to create_provider that includes the timeout
    provider_args = {}

    # Add provider-specific options based on args
    if args.provider == "ollama" and hasattr(args, "timeout") and args.timeout:
        # Add timeout directly to provider arguments
        provider_args["request_timeout"] = args.timeout
        logger.info(f"Setting custom timeout for Ollama provider: {args.timeout}s")

    # If we have provider-specific args, we need to create the provider directly
    if provider_args and args.provider == "ollama":
        # Import the specific provider class
        from atlas.providers.implementations.ollama import OllamaProvider

        # Create provider directly with all arguments
        return OllamaProvider(model_name=args.model, **provider_args)
    else:
        # Resolve options and use standard provider creation
...
def handle_example_error(
    error: Exception, message: str = None, user_message: str = None, exit_code: int = None
) -> None:
...
def create_providers_for_examples(
    provider_names: Optional[List[str]] = None, ensure_mock: bool = True, min_providers: int = 1
) -> List[Any]:
    ...
    """
    from atlas.providers import ModelProvider, create_provider, discover_providers
    ...
    for provider_name, models in available.items():
        if provider_name != "mock" or (
            provider_name == "mock" and not any(p.name == "mock" for p in providers)
        ):
            try:
    ...
    if len(providers) < min_providers:
        ...
        if (
            len(providers) == 1
            and providers[0].name == "mock"
            and providers[0].model_name == "mock-standard"
        ):
...
def create_provider_group_for_examples(
    ...
    providers = create_providers_for_examples(
        provider_names=provider_names, ensure_mock=True, min_providers=min_providers
    )

    # Create provider group with the specified strategy
    return create_provider_group(
        providers=providers, selection_strategy=selection_strategy, name=name
    )
