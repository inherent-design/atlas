...
def count_streaming_callback(delta: str, full_text: str) -> None:
    ...
    if len(delta) > 1:
        print(f"[Received ~{len(delta)} chars, ~{tokens_estimate} total tokens]", end="\r", flush=True)
...
def add_example_arguments(parser):
    ...
    parser.add_argument(
        "--query", 
        type=str, 
        help="Query to use instead of example queries"
    )
    parser.add_argument(
        "--delay", 
        type=float,
...
def main():
    ...
    try:
        ...
        logger.info(f"Initializing Atlas query client with {provider.name} provider and {provider.model_name} model...")
    ...
    query = args.query or "Explain how Atlas can be integrated with other systems in a step-by-step guide."
    print(f"Query: {query}")
